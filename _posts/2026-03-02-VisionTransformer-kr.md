...
이전까지 큰 스케일의 이미지 작업은 ResNet과 같은 CNN 계열의 모델이 최고 수준의 성능을 보여왔었습니다. 그러나, NLP에서 Transformer 구조가 성능을 혁신하며 SOTA를 갱신한 것 처럼, Transformer의 인코더 구조를 최소한의 변화만으로 이미지 분류 문제에도 적용시켜 좋은 성능을 이끌어 낼 수 있는지에 대한 논문입니다.

<p align="center">
  <img src="{{ site.baseurl }}/assets/images/posts/VisionTransformer/1.jpg" width="70%">
</p>
...
그러나 필자는 모델이 더 큰 데이터셋으로 학습 하였을 때, inductive bias 문제를 극복 가능하다고 합니다. 따라서 ViT(Vision Transformer)는 충분히 큰 데이터셋에서 Pre-trained 된 모델을 작은 dataset으로 전이학습(Transfer learning) 시켰을때 좋은 성능을 보였습니다.
...
<p align="center">
  <img src="{{ site.baseurl }}/assets/images/posts/VisionTransformer/2.jpg" width="70%">
</p>
